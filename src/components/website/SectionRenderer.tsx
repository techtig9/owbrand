    ...
    case 'hero':
      return (
        <section className="px-8 py-16 text-center">
          {content.eyebrow && (
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-faint">{content.eyebrow}</p>
          )}
          <h1 className="mx-auto mt-3 max-w-3xl font-display text-4xl font-bold leading-tight text-ink">
            {content.headline}
          </h1>
          {content.subheadline && (
            <p className="mx-auto mt-4 max-w-xl text-base leading-7 text-ink-soft">{content.subheadline}</p>
          )}
        ...
        <section className="px-8 py-14">
          ...
          <div className="mx-auto mt-8 grid max-w-5xl gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {content.items.map((item, i) => (
              <article key={i} className="rounded-2xl border border-line bg-white p-5">
                <h3 className="font-display text-base font-semibold text-ink">{item.title}</h3>
                {item.description && <p className="mt-2 text-sm leading-6 text-ink-soft">{item.description}</p>}
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="px-8 py-14">
          ...
          <div className="mx-auto mt-8 grid max-w-5xl gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {(content.productIds.length > 0 ? content.productIds : ['', '', '']).map((id, i) => (
              <div key={i} className="rounded-2xl border border-dashed border-line bg-canvas-alt p-5">
    ...
    case 'testimonials':
      ...
      return (
        <section className="px-8 py-14">
          ...
          <div
            role="note"
            className="mx-auto mt-4 max-w-2xl rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900"
          >
            {content.note}
          </div>
          <div className="mx-auto mt-6 grid max-w-5xl gap-5 sm:grid-cols-3">
            {Array.from({ length: content.slots }).map((_, i) => (
              <div key={i} className="rounded-2xl border border-dashed border-line bg-canvas-alt p-5">
                ...
                <p className="mt-4 text-xs text-ink-faint">Your customer’s words go here</p>
              </div>
            ))}
          </div>
        </section>
            ...
            {content.tiers.map((tier, i) => (
              <article
                ...
                className={`rounded-2xl border p-5 ${
                  tier.highlighted ? 'border-ink bg-white shadow-soft' : 'border-line bg-white'
                }`}
              >
                ...
                {tier.price && (
                  <p className="mt-2 font-display text-2xl font-bold tabular-nums text-ink">
                    {tier.price}
                    {tier.cadence && <span className="text-sm font-normal text-ink-faint">/{tier.cadence}</span>}
                  </p>
                )}
                {tier.description && <p className="mt-2 text-xs leading-5 text-ink-soft">{tier.description}</p>}
                {tier.features.length > 0 && (
                  <ul className="mt-4 space-y-1.5">
                    {tier.features.map((feature, j) => (
                      <li key={j} className="text-xs leading-5 text-ink-soft">
                        {feature}
                      </li>
                    ))}
                  </ul>
      ...
      return (
        <section className="px-8 py-14">
          <div className="mx-auto max-w-xl text-center">
            <SectionHeading heading={content.heading} />
            {content.body && <p className="mt-3 text-sm leading-6 text-ink-soft">{content.body}</p>}
            {content.showForm && (
      ...
      return (
        <footer className="border-t border-line px-8 py-10">
          <div className="mx-auto flex max-w-5xl flex-col gap-6 sm:flex-row sm:justify-between">
            {content.tagline && <p className="text-sm text-ink-soft">{content.tagline}</p>}
...
function SectionHeading({ heading, subheading }: { heading?: string; subheading?: string }) {
  ...
  return (
    <div className="mx-auto max-w-2xl text-center">
      ...
      {subheading && <p className="mt-2 text-sm leading-6 text-ink-soft">{subheading}</p>}
    </div>
  );
}
...
function ImagePlaceholder({ prompt, className = '' }: { prompt: string; className?: string }) {
  return (
    <div
      className={`flex items-center justify-center rounded-2xl border border-dashed border-line bg-canvas-alt p-4 ${className}`}
    >
      <p className="max-w-xs text-center text-xs leading-5 text-ink-faint">
...
function UnknownSection({ value }: { value: unknown }) {
  return (
    <section className="px-8 py-8">
      <div className="rounded-xl border border-dashed border-line bg-canvas-alt p-4 text-xs text-ink-faint">
