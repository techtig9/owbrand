  ...
  return (
    <div className="rounded-2xl border border-line bg-surface p-5">
      ...
      {measured ? (
        ...
        <p className="mt-1 font-display text-2xl font-bold text-content-tertiary" title="This platform did not report this metric">
          —
        </p>
      )}
      ...
      {!measured && <p className="mt-1 text-[11px] leading-4 text-content-tertiary">Not reported by the platform</p>}
...
function Delta({
  ...
  if (delta.direction === 'new') {
    return (
      <p className="mt-1.5 text-[11px] leading-4 text-content-tertiary">
  ...
  if (delta.percent === null || delta.direction === 'flat') {
    return (
      <p className="mt-1.5 inline-flex items-center gap-1 text-[11px] font-medium text-content-tertiary">
  ...
  return (
    <p
      className={`mt-1.5 inline-flex items-center gap-1 text-[11px] font-semibold ${
        good ? 'text-success' : 'text-primary'
      }`}
    >
      ...
      <span className="font-normal text-content-tertiary">vs {delta.previousLabel}</span>
    </p>
  );
}
...
export function MagnitudeBars({
  ...
  if (rows.length === 0) {
    return (
      <p className="rounded-xl border border-dashed border-line bg-surface-raised px-4 py-8 text-center text-sm text-content-tertiary">
        ...
        <li key={row.label}>
          <div className="flex items-baseline justify-between gap-3">
            ...
            <span className="text-sm tabular-nums text-content-secondary">
              ...
              {row.secondary && <span className="ml-2 text-xs text-content-tertiary">{row.secondary}</span>}
            </span>
          </div>
          <div className="mt-1.5 h-2.5 overflow-hidden rounded-full bg-surface-raised">
            <div
