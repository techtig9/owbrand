...
function LogoCard({ accent }: { accent: string }) {
  return (
    <div className={`flex h-full flex-col items-center justify-center gap-3 rounded-xl ${accent} p-6`}>
      <div className="flex h-14 w-14 items-center justify-center rounded-full bg-surface/80 font-display text-2xl font-bold text-ink shadow-soft">
        Ow
      </div>
      <div className="h-2 w-16 rounded-full bg-surface/70" />
    </div>
  );
}
...
function ReelCard({ accent }: { accent: string }) {
  return (
    <div className={`relative flex h-full flex-col justify-end rounded-xl ${accent} p-3`}>
      <div className="absolute right-3 top-3 rounded-full bg-surface/80 px-2 py-0.5 text-[10px] font-semibold text-ink">
        ● REC
      </div>
      <div className="space-y-1.5 rounded-lg bg-surface/70 p-2 backdrop-blur">
...
function MockCard({ kind, tilt, label, accent }: (typeof CARDS)[number]) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 px-1 text-[11px] font-medium text-content-secondary">{label}</p>
    </div>
  );
}
