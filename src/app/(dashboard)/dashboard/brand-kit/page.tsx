  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-soft">{brand.description}</p>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-line bg-white p-6">
          <h2 className="font-display text-sm font-semibold text-ink">Logo</h2>
          <div className="mt-4 flex h-32 items-center justify-center rounded-xl bg-canvas-alt">
            {brand.logo_url ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={brand.logo_url} alt={`${brand.name} logo`} className="max-h-20" />
            ) : (
              <span className="text-xs text-ink-faint">Generate a logo in the Content Studio</span>
            )}
          </div>
        </div>
            ...
            {brand.brand_colors?.length ? (
              brand.brand_colors.map((color: string) => (
                <div key={color} className="text-center">
                  ...
                  <p className="mt-1 text-[10px] text-ink-faint">{color}</p>
                </div>
              ))
            ) : (
              <span className="text-xs text-ink-faint">No palette generated yet</span>
            )}
          </div>
        </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {brand.brand_fonts?.length ? (
              brand.brand_fonts.map((font: string) => (
                <span key={font} className="rounded-full border border-line bg-canvas-alt px-3 py-1 text-xs text-ink-soft">
                  {font}
                </span>
              ))
            ) : (
              <span className="text-xs text-ink-faint">No fonts generated yet</span>
            )}
          </div>
        </div>
      </div>
