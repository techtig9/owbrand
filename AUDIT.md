...
## Phase 6 — growth and conversion

**Three false claims removed from the landing page.** The FAQ advertised a reel
compositing layer, a built-in code editor and one-click deployment. None exist.
A marketing page is the one place in a codebase where a false claim has no
compiler, no test and no reviewer, so it survives every refactor that
invalidates it. All seven answers are now checkable against the code.

**`AIDemo` had three dead controls** — a Generate button, a microphone and three
device tabs, all under a heading promising a demonstration. Replaced with a real
rate-limited demo when the deployment can run one, and a static preview
carrying no interactive controls when it cannot.

**The skip link was broken on six pages.** The landing page's `<main>` had no id
and `AuthShell` used a `<div>`. Both existing assertions passed throughout,
which is why the new one checks the target resolves and is a landmark.

Built this phase: sitemap and robots from one shared list, JSON-LD derived from
the live plan and FAQ data, a generated OG image and favicon (the app shipped
no icon at all), HowItWorks, an honest Differentiation section including "when
owbrand is the wrong choice", an activation checklist derived from row counts,
a usage meter whose upgrade prompt is threshold-aware, a feedback widget, a
referral programme with abuse controls proven against real Postgres, a
changelog that lists fixes and security changes rather than only features, an
MDX blog with three drafts, and five legal drafts.

Two design decisions worth recording:

- **The legal pages' draft notice is rendered by the page component, not stored
  in the content.** A warning that lives in the data can be deleted by editing
  the data, and the person most likely to do that is someone who wants the page
  to look finished. They also set `robots: noindex` and are excluded from the
  sitemap — an unreviewed privacy policy that ranks is worse than one nobody
  can find, and a sitemap advertising a noindex page is a contradiction a
  crawler reports back to you.
- **The footer throws at module scope** if a `/legal/*` route in the sitemap
  list is missing from its links. A legal page that is reachable to a crawler
  but not to a human is a compliance problem, not a navigation one.

Also fixed: the export archive hard-coded "Generated by owbrand
(https://owbrand.ai)" into **every** plan's README — the opposite of the
intended attribution policy, and pointing at production from any deployment.

Verified live: all seven new routes 200; the sitemap lists three URLs and
excludes the five drafts; the drafts serve `noindex, follow` and the notice;
the changelog renders 7 added, 3 fixed, 3 security, 1 changed; the blog renders
its empty state because all three posts are drafts.

**559 unit tests (36 files), 149/149 database, 169/169 browser, zero lint
warnings, contrast gate and build all pass.**
